...
const templatesDir = path.join(__dirname, '..', 'templates/middleware'); // templates文件夹的路径
...
// 创建目录和文件
const directoryParts = middlewareName.split('/');
const directory = path.join(__dirname, '..', 'middlewares', ...directoryParts.slice(0, -1));
...
if (fs.existsSync(filePath)) {
  ...
  // 替换模板中的占位符
  defaultCode = defaultCode.replace(/_middlewareName/g, middlewareName);
